...
import { IssueCategory, IssueSeverity, PhaseName } from "../../types.js";
...
export async function runLayoutProbe(session: ActiveScan, page: Page): Promise<void> {
  ...
  const result = await page.evaluate((shim: string) => {
    eval(shim);
    ...
    const styledProbe = Array.from(
      document.querySelectorAll("button, .ant-btn, .thy-btn, a.thy-nav-item, thy-menu-item"),
    ).slice(0, 30);
    let unstyled = 0;
    for (const el of styledProbe) {
    ...
    if (styledProbe.length >= 5 && unstyled / styledProbe.length > 0.7) {
      findings.push({
        title: "CSS 疑似未生效",
        detail: `抽样 ${styledProbe.length} 个 UI 元素中 ${unstyled} 个缺少常见样式`,
        severity: IssueSeverity.Error,
      });
    }
